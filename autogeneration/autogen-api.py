...
#TODO
#    if (is_list_type == True) and (card_field == 'noCardinality'):
#            nlg.write(tab + tab + 'assert isinstance(' + prop + ', "' + 'str' + '),\\' + '\n')
#            nlg.write(tab + tab + '"[' + f_name + '] ' + prop + ' must be of type ' + p_type + '."' + '\n')
#            body_assert_types += 'REQUIRED    XSD     SINGLE    ' + prop + '\n'
    ...
    if is_list_type == False:
        ...
        if check_type == 'CASE-Object':
            ...
            nlg.write(tab + tab + 'assert XSDValidator.validateXSD(' + prop + ', "' + p_type + '", autogenerated_xsd),\\' + '\n')
            nlg.write(tab + tab + '"[' + f_name + '] ' + prop + ' must be of type List of ' + p_type + '."' + '\n')
            body_assert_types += 'REQUIRED    XSD       LIST    ' + prop + '\n'
...
# USE DEBUG_PRINT STATEMENTS HERE RATHER THAN BODY ASSERTS (for same block of code).
        for prop in p_list:
            ...
            if prop_card_dict == None:    #Skips GENERAL AXIOMS in Turtle file (v0.1.0) and undefined props.
                continue
            else:
...
#                print 'REQUIRED WRITE'
                ...
                # These are written in the next block (copy of this block but with p_required == False.
                continue
